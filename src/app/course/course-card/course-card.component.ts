...
export class CourseCardComponent implements OnInit {
  ...
  constructor(
    ...
    private courseserve: CourseserveService,
    private userService: UserService,
    private router: Router,
    private student: StudentserveService
  ) {}
  user!: any;
  action!: boolean;
  ids!: any;
  ...
  enroll() {
    ...
    if (!this.action) {
      ...
      this.ids = {
        ...
        course_id: this.coursesfrompare.id,
      };
  ...
  ifenroll() {
    ...
    this.userService.ifenroll(this.ids).subscribe((res) => {
      if (res == 0) this.permission = 'Please Enroll to Access Course Content';
      else {
        if (res == 1)
          this.router.navigateByUrl(
            `course-content/${this.coursesfrompare.id}`
          );
